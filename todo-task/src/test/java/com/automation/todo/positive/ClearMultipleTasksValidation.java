...
public class ClearMultipleTasksValidation extends Tests {
	...
	public void preTestExecution() {
		...
		log = (Logger) LogManager.getLogger(ClearMultipleTasksValidation.class);
	...
	@Test
	public void ClearMultipleTasksValidationTest() throws Exception {
		...
		/*
		 * Step3 : Click clear completed button
		 * 
		 */

		todoPage.clickClearCompletedButton(className);

		
		/*
		 * Step4 : Validate tasks are removed form
		 * 
		 */

		for (int i = 0; i < itemsToDelete.length; i++) {

			Assert.assertEquals(todoPage.validateTasksRemovedFromList(itemsToDelete[i]), false,
					"Task is not removed from List");
